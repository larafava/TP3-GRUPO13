...
template <class T1>
void cLista<T1>::Switch(unsigned int pos1, unsigned int pos2) {
	T1* aux = NULL;
	if (pos1 > t || pos2 > t)
...
template <class T1>
T1* cLista<T1>::Quitar(int pos)
{
	unsigned int i = BuscarAtPos(pos);
	if (i >= ca)return NULL;
	T1 *aux = NULL;
	aux = lista[i];
	ca--;
...
template <class T1>
T1* cLista<T1>::QuitarPos(unsigned int pos) {
...
template <class T1>
void cLista<T1>::Eliminar(int pos)
{
	T1 *aux = Quitar(pos);
	if (aux != NULL)
		delete aux;
...
template <class T1>
 int cLista<T1>::BuscarAtPos(T1*pos)
{
	for (int i = 0; i < ca; i++)
	{
		if (lista[i] == pos)
